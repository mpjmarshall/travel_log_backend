...
// TripWrite is the body of PUT /v1/trips/{id}: a whole-state upsert of the
// seven fields the route owns.
...
func ValidateTrip(t TripWrite) error {
	...
	if t.Summary != nil && len(*t.Summary) > MaxSummaryBytes {
		return InvalidFieldError{Field: "summary",
			Why: fmt.Sprintf("%d bytes, and this build takes at most %d", len(*t.Summary), MaxSummaryBytes)}
	}
	if err := checkCityIDs(t.CityIDs); err != nil {
		return err
	}
	if t.CoverAsset != nil && !assetPattern.MatchString(*t.CoverAsset) {
		return InvalidFieldError{Field: "coverAsset",
			Why: "a cover is a media object id: 64 lowercase hex characters"}
	}
	if t.Start != nil && t.End != nil && t.End.Time().Before(t.Start.Time()) {
